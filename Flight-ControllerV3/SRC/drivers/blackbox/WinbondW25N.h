...
class W25N {
  private:
     ...
     */
    uint8_t readReg(uint8_t* reg, void *pBuf, uint16_t rxSize, uint16_t txSize=1);

    void multiWrite(void *pBuf1, void *pBuf2, size_t size1, size_t size2);
  ...
  public:
    ...
    void setStatusReg(uint8_t reg, uint8_t data);
     ...
     * */
    bool blockErase(uint32_t pageAdd);
    ...
    bool bulkErase(uint32_t startAddr);
    ...
    int loadProgData(uint16_t columnAdd, uint8_t* buf, uint32_t dataLen, uint32_t pageAdd);
     ...
     * WILL ERASE THE DATA IN BUF OF LENGTH DATALEN BYTES
     */
    int loadRandProgData(uint16_t columnAdd, uint8_t* buf, uint32_t dataLen);
    int loadRandProgData(uint16_t columnAdd, uint8_t* buf, uint32_t dataLen, uint32_t pageAdd);
    ...
    void readBBM(uint16_t* bbm);

    uint16_t getBadPageAddress();
    ...
    uint16_t getBadBlocks(uint16_t* badBlocks);

    void loadProgAndExecute(uint16_t page, uint16_t startAddr, uint8_t* buff, uint16_t dataLen);

    bool readPage(uint16_t page, uint16_t startAddr, uint8_t* buff, uint16_t dataLen);
