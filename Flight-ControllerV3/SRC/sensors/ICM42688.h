...
typedef struct accCalibData {
	float scale[3][3] = {{9.963706, 0.000776, -0.003530},
					   {0.000776, 9.983607, 0.007309},
					   {-0.003530, 0.007309, 9.958600}};
	Vector_t<float> offset = {-0.000231, 0.000062, -0.000586};
}accCalibData_t;

typedef struct calibDefault {
	const float accB[3] = {-0.00190067291, 0.00148680806, 0.00142973661};
	const float accS[3] = {0.997095108, 0.998611927, 0.998517811};
}calibDefault_t;
...
public:
   ...
   */
  int init(SPI_HandleTypeDef *hspi, GPIO_TypeDef *csPort, uint16_t csPin, uint32_t SPI_HS_CLK=8000000);
  ...
  Vector_t<float> getAccelData(void);
  ...
  Vector_t<float> getGyroData(void);
